...
Solving the biharmonic problem on a 1x1 unit square 
'''
...
def dirichlet_bi(T,q,Td,domain,ds,h, C_T_0=10, C_T_1 = 10):
    n = ufl.FacetNormal(domain)
    const0 = ((T-Td)*ufl.inner(ufl.grad(lap(q)), n))*ds
    adjconst0= (q*ufl.inner(ufl.grad(lap(T)), n))*ds
    pen0 = ((C_T_0/(h**3))*q*(T-Td))*ds 
    const1 = -( lap(q)*(ufl.inner(ufl.grad(T), n) - ufl.inner(ufl.grad(Td), n))) *ds
    adjconst1= - (lap(T)*ufl.inner(ufl.grad(q), n))*ds
    pen1 = ((C_T_1/h)*(ufl.inner(ufl.grad(T), n) - ufl.inner(ufl.grad(Td), n))*ufl.inner(ufl.grad(q), n))*ds 
    return pen0 + pen1 + const0 + adjconst0 + const1 + adjconst1
...
parser.add_argument('--of',dest='of',default="biharmonicIntData.csv",
...
if mesh_exists:
    ...
    RKPMBasis.makePoints(gridType=gridType, meshEps=gridEps, corners=corners, edgeNodes=edgenodes)
...
f = lap(lap(u_ex))
res_T =  ufl.inner(lap(u),lap(v))*(dx_custom) - ufl.inner(v,f)*(dx_custom)
resD_T_l = dirichlet_bi(u,v,u_ex,domain,ds(left_ID),h)
resD_T_r = dirichlet_bi(u,v,u_ex,domain,ds(right_ID),h)
resD_T_t = dirichlet_bi(u,v,u_ex,domain,ds(top_ID),h)
resD_T_b = dirichlet_bi(u,v,u_ex,domain,ds(bottom_ID),h)
...
H2 = fem.assemble_scalar(fem.form(ufl.inner(lap(u-u_ex), lap(u-u_ex)) * dx_custom))
H2_assembled  = domain.comm.allreduce(H2, op=MPI.SUM)
...
H2_norm = fem.assemble_scalar(fem.form(ufl.inner(lap(u_ex), lap(u_ex)) * dx_custom))
H2_assembled_norm  = domain.comm.allreduce(H2_norm, op=MPI.SUM)

L2_error = np.sqrt(L2_assembled / L2_assembled_norm)
H10_error = np.sqrt(H1_assembled / H1_assembled_norm)
H20_error = np.sqrt(H2_assembled / H2_assembled_norm)
sum_T = np.sqrt(T_sum_assembled)

print(f"Error L2: {L2_error}")
print(f"Error H10: {H10_error}")
print(f"Error H20: {H20_error}")
print(f"Sum T: {sum_T}")
...
if write_file: 
    ...
    fs = str(bg_dofs)+","+str(fg_dofs)+ "," + str(ref_val)+","+str(n)+","\
        + str(L2_error)+","+str(H10_error)+","+str(H20_error)+","\
